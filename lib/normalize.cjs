...
function normalizeKey(rawText, importe) {
  ...
  // Devoluciones a la tarjeta: el banco escribe SIEMPRE el mismo texto
  // ("ABONO EN LA TARJETA 5489... EL 2026-07-22") y lo único que cambia es el
  // número de tarjeta y la fecha, que la limpieza de más abajo quita a
  // propósito. Resultado: tres devoluciones de tres compras que no tienen nada
  // que ver acababan en el mismo grupo. No son un proveedor ni un gasto
  // recurrente -- cada una va suelta, así que la fecha y el importe entran en
  // la clave para que no se junten.
