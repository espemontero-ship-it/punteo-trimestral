...
// en rutas API (Node) como en middleware/proxy (Edge) sin distinción de runtime.
...
// Contraseña única de administradora (Esperanza), fija en variable de entorno.
...
// payload: { rol: 'admin' } o { rol: 'colaborador', colaboradorId }
async function crearTokenSesion(payload) {
  const expira = Date.now() + DURACION_MS;
  return sign(JSON.stringify({ ...payload, exp: expira }));
}

// Devuelve el payload de la sesión (con rol/colaboradorId) si es válida, o null.
...
// Helper para rutas API: lee y valida la sesión a partir del request de Next.js.
async function obtenerSesion(request) {
  const token = request.cookies.get(SESSION_COOKIE)?.value;
  return leerSesion(token);
}

module.exports = { verificarPassword, crearTokenSesion, leerSesion, obtenerSesion, SESSION_COOKIE, DURACION_MS };
