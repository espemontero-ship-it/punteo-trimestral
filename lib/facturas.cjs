...
async function analizarBuffer(buffer, esPdf) {
  ...
  try {
    ...
    return {
      ...
      // El texto entero, para poder mirar después por qué se eligió un número
      // y no otro. Se corta a 20.000 caracteres: una factura normal no llega
      // ni de lejos, y evita guardar un tocho si alguien sube un PDF de 200
      // páginas por error. Se quitan los bytes nulos porque Postgres no los
      // admite en una columna de texto y la guardada falla entera (pasó con
      // tres facturas reales al rellenar las antiguas).
      texto: resultado.text.replace(/\u0000/g, '').slice(0, 20000),
    };
  } catch (err) {
...
// Una sola implementación, con una sola forma de llamarla: las dos listas.
// Había otra con el mismo nombre en facturaMatcher.cjs que recibía dos
// argumentos en vez de un objeto. Cada una se usaba en su sitio, pero
// llamar a la que no toca no da error: devuelve null, que significa "no he
// reconocido ningún importe", y esa factura se queda sin cruzar en silencio.
