...
// proveedor) no tiene nada con qué comparar y todo cae en "ambiguo".
const MESES = {
...
const RE_FECHA_MES = new RegExp(
  `(?:(${NOMBRES_MES})\\s+(\\d{1,2}),?\\s+(\\d{4})|(\\d{1,2})\\s+(${NOMBRES_MES})[,.]?\\s+(\\d{4}))`,
  'gi'
);
...
function extraerCandidatos(texto) {
  ...
  while ((m = RE_FECHA_MES.exec(texto)) !== null) {
    const [, mesA, diaA, anioA, diaB, mesB, anioB] = m;
    const mesNombre = (mesA || mesB).toLowerCase();
    const mesNum = MESES[mesNombre];
    if (!mesNum) continue;
    const dia = (diaA || diaB).padStart(2, '0');
    const anio = anioA || anioB;
