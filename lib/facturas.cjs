...
// Número con formato español (coma decimal, punto opcional de miles) junto al
// símbolo/código de moneda, en cualquier orden y con espacio opcional.
const RE_MONEDA = /(\d{1,3}(?:\.\d{3})*,\d{2})\s*(?:€|EUR)\b|(?:€|EUR)\s*(\d{1,3}(?:\.\d{3})*,\d{2})/gi;
...
const RE_NUMERO_SOLO = /\d{1,3}(?:\.\d{3})*,\d{2}/g;
const RE_LINEA_TOTAL = /\b(total|importe|a pagar|total factura)\b/i;
...
function extraerCandidatos(texto) {
  ...
  for (const linea of texto.split('\n')) {
    const esLineaTotal = RE_LINEA_TOTAL.test(linea);
    let m;
    const re = new RegExp(RE_MONEDA.source, 'gi');
    let encontradoConMoneda = false;
    while ((m = re.exec(linea)) !== null) {
      ...
      encontradoConMoneda = true;
    }
    // Sin € en la línea pero suena a total (p.ej. el símbolo cayó en otra
    // celda de la tabla al extraer el texto): coger el número igualmente.
    if (!encontradoConMoneda && esLineaTotal) {
      const reNum = new RegExp(RE_NUMERO_SOLO.source, 'g');
      while ((m = reNum.exec(linea)) !== null) {
