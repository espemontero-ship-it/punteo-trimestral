...
async function fusionarHoja(hoja, filasNuevas, importacionId) {
  ...
  for (let i = 0; i < filasAInsertar.length; i += TAMANO_TANDA) {
    const tanda = filasAInsertar.slice(i, i + TAMANO_TANDA);
    const valores = [];
    const marcadores = tanda.map((fila, j) => {
      const base = j * COLUMNAS;
      valores.push(
        hoja, fila.fila, fila.fecha, fila.concepto, fila.importe, fila.clave,
        JSON.stringify(fila.datosOriginales), importacionId
      );
  ...
  return { actualizadas, insertadas: filasAInsertar.length };
}
