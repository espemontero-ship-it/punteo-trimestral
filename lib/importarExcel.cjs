...
async function listarImportaciones() {
  const { rows } = await query(
    `SELECT i.id, i.hoja, i.nombre_archivo, i.creado_en,
            COUNT(m.id) AS total,
            COUNT(m.id) FILTER (WHERE m.estado = 'resuelta') AS resueltas
     FROM importaciones i
     LEFT JOIN movimientos m ON m.importacion_id = i.id
     GROUP BY i.id
     ORDER BY i.creado_en DESC`
  );
  return rows.map(r => ({
    id: r.id, hoja: r.hoja, nombreArchivo: r.nombre_archivo, creadoEn: r.creado_en,
...
async function borrarImportacion(importacionId) {
  const { rows: existentes } = await query(
    `SELECT ruta_blob FROM importaciones WHERE id = $1`,
    [importacionId]
  );
