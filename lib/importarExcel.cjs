  ...
  for (const fila of filasNuevas) {
    ...
    if (ya) {
      ...
      await query(
        ...
        [fila.fila, fila.clave, JSON.stringify(fila.datosOriginales), importacionId, ya.id]
      );
...
module.exports = {
  mismaLinea, fusionarHoja, importarMovimientos, recalcularClaves, listarImportaciones, borrarImportacion };
