...
async function vincularMovimiento(pagoId, movimientoId) {
  ...
  const { rows: facturaRows } = await query(
    `SELECT f.numero FROM pago_facturas pf JOIN facturas f ON f.id = pf.factura_id WHERE pf.pago_id = $1 ORDER BY f.numero`,
    [pagoId]
  );
  const numeros = facturaRows.map(r => r.numero).filter(n => n !== null);
  const nota = numeros.length > 0
    ? numeros.join(' + ')
    : `${colaborador_nombre} - ${evento}`;
