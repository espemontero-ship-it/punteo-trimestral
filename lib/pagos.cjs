...
// Vincula el pago a una línea del banco ya existente. La nota final de esa
// línea lista los números de factura que justifica (o queda genérica si el
// pago todavía no tiene facturas asociadas, ej. un anticipo).
async function vincularMovimiento(pagoId, movimientoId) {
  ...
  // La nota dice QUÉ es esta línea; los números de factura ya no van aquí --
  // viven en la columna Factura, que es su sitio.
  const nota = `${colaborador_nombre} - ${evento}`;

  await query(`UPDATE pagos SET movimiento_id = $1 WHERE id = $2`, [movimientoId, pagoId]);

  // Se enlazan además las facturas con la propia línea del banco. Sin esto,
  // la columna Factura de esa línea salía vacía en pantalla y en el excel de
  // la gestoría, aunque el pago sí supiera qué facturas justificaba.
