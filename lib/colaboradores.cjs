...
async function listarColaboradores() {
  const { rows } = await query(`SELECT id, nombre, usuario, creado_en FROM colaboradores ORDER BY nombre`);
  return rows;
}

module.exports = { crearColaborador, verificarColaborador, listarColaboradores };
