...
async function obtenerColaborador(colaboradorId) {
  const { rows } = await query(
    `SELECT id, nombre, usuario, estado, puede_invitar FROM colaboradores WHERE id = $1`,
    [colaboradorId]
  );
  return rows[0] || null;
}
...
// aceptar una invitación, nunca generada/dictada) y su lote para el proyecto
// al que se le invitó.
async function crearColaboradorDesdeInvitacion(nombre, usuario, passwordHash, proyecto) {
...
module.exports = {
  ...
  actualizarEstadoColaborador, actualizarPermisoInvitar, crearColaboradorDesdeInvitacion,
};
