...
async function crearColaboradorDesdeInvitacion(nombre, usuario, passwordHash, proyecto, puedeSubirFacturasGenerales) {
  const { rows } = await query(
  ...
  if (proyecto) {
    ...
    loteId = await crearLote(trimestreActual(), colaborador.id, proyecto);
  }
