...
// solo se actualiza (nuevo lote y/o permiso de NOL); si no existe, se manda
// invitación por correo para que elija su propia contraseña. Proyecto y
// "sube facturas de NOL" son independientes -- nunca una elección excluyente,
// un colaborador de proyecto puede además subir facturas que paga NOL directamente.
async function altaColaborador({ nombre, usuario, proyectoId, puedeSubirFacturasGenerales, invitadoPor }) {
  if (!proyectoId && !puedeSubirFacturasGenerales) {
    throw Object.assign(new Error('Elige un proyecto o marca que suba facturas de NOL.'), { status: 400 });
  }
  ...
  if (existentes.length > 0) {
    ...
    return { yaExistia: true, colaborador, loteId };
  }

  let proyectoNombre = null;
  if (proyectoId) {
    const { rows } = await query('SELECT nombre FROM proyectos WHERE id = $1', [proyectoId]);
    proyectoNombre = rows[0]?.nombre || null;
  }
  const token = await crearInvitacion({ nombre, usuario, proyectoId: proyectoId || null, invitadoPor, puedeSubirFacturasGenerales: !!puedeSubirFacturasGenerales });
  const enlace = `${process.env.SITE_URL || ''}/invitacion/${token}`;
  await enviarInvitacion(usuario, { nombre, proyecto: proyectoNombre, enlace });
