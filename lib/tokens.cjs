...
async function crearInvitacion({ nombre, usuario, proyecto, invitadoPor }) {
  const token = generarToken();
  const expiraEn = new Date(Date.now() + DURACION_INVITACION_MS);
  await query(
    `INSERT INTO tokens_acceso (tipo, token_hash, nombre, usuario, proyecto, invitado_por, expira_en)
     VALUES ('invitacion', $1, $2, $3, $4, $5, $6)`,
    [hashToken(token), nombre, usuario, proyecto, invitadoPor || null, expiraEn]
  );
  return token;
}
...
async function verTokenPendiente(token, tipo) {
  const { rows } = await query(
    `SELECT id, nombre, usuario, proyecto, colaborador_id FROM tokens_acceso
...
async function consumirToken(token, tipo) {
  const { rows } = await query(
    `UPDATE tokens_acceso SET usado_en = now()
     WHERE token_hash = $1 AND tipo = $2 AND usado_en IS NULL AND expira_en > now()
     RETURNING id, nombre, usuario, proyecto, colaborador_id, invitado_por`,
    [hashToken(token), tipo]
  );
  return rows[0] || null;
}
