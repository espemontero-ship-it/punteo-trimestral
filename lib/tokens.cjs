...
// `proyecto` es opcional -- una invitación sin proyecto es para alguien que
// solo va a subir facturas generales de NOL, sin ningún lote.
async function crearInvitacion({ nombre, usuario, proyecto, invitadoPor, puedeSubirFacturasGenerales }) {
  const token = generarToken();
  const expiraEn = new Date(Date.now() + DURACION_INVITACION_MS);
  await query(
    `INSERT INTO tokens_acceso (tipo, token_hash, nombre, usuario, proyecto, invitado_por, expira_en, puede_subir_facturas_generales)
     VALUES ('invitacion', $1, $2, $3, $4, $5, $6, $7)`,
    [hashToken(token), nombre, usuario, proyecto || null, invitadoPor || null, expiraEn, !!puedeSubirFacturasGenerales]
  );
  return token;
}
...
async function verTokenPendiente(token, tipo) {
  const { rows } = await query(
    `SELECT id, nombre, usuario, proyecto, colaborador_id, puede_subir_facturas_generales FROM tokens_acceso
...
async function consumirToken(token, tipo) {
  const { rows } = await query(
    `UPDATE tokens_acceso SET usado_en = now()
     WHERE token_hash = $1 AND tipo = $2 AND usado_en IS NULL AND expira_en > now()
     RETURNING id, nombre, usuario, proyecto, colaborador_id, invitado_por, puede_subir_facturas_generales`,
    [hashToken(token), tipo]
  );
  return rows[0] || null;
}
