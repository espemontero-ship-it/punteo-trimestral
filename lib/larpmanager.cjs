...
async function listarPagosLarpManagerSinEmparejar() {
  ...
  return rows.map(p => {
    ...
    if (p.movimiento_id) {
      return {
        ...p, motivo: 'emparejado',
        motivoTexto: `Emparejado con la línea del ${conFecha({ fecha: p.movimiento_fecha }, p.fecha)}, de ${eur(p.movimiento_importe)}: «${p.movimiento_concepto || ''}»`,
      };
    }
    ...
    if (p.estado === 'resuelta') {
      return { ...p, motivo: 'resuelta_a_mano', motivoTexto: 'Dado por bueno a mano, sin línea del banco.' };
    }
    ...
    if (porApellido.length > 0) {
      ...
      return {
        ...
        motivoTexto: `Hay ${ordenadas.length} línea(s) de ${eur(p.importe)} a su nombre (${ordenadas.map(m => `${conFecha(m, p.fecha)}${quienLaTiene(m, p)}`).join('; ')}). O falta esta transferencia por llegar, o alguno de esos pagos está de más.`,
      };
    }
...
async function historialDeJugador(pagoId) {
  ...
  return rows.map(r => {
    ...
    if (!r.movimiento_id) {
      ...
      else if (r.estado === 'resuelta') nota = 'Dado por bueno a mano, sin línea del banco';
...
async function cambiarEstadoPago(pagoId, estado) {
  ...
  if (rows[0].movimiento_id) {
    throw new Error('Ese pago tiene línea del banco: quítale el vínculo antes de cambiarle el estado.');
  }
...
module.exports = {
  ...
  __norm: normalizar, __var: normalizarVariantes,
};
