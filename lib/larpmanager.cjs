...
function repartirUnoAUno(lineas, pagos) {
  ...
  for (const l of lineas) {
    for (const p of pagos) {
      if (!tokensCoinciden(p.variantes, l.n)) continue;
...
async function listarPagosLarpManagerSinEmparejar() {
  ...
  if (pagosTodos.length > 0) {
    ...
    const { porPago } = repartirUnoAUno(
      ...
      pagosTodos.map(p => ({ clave: p.id, importe: p.importe, fecha: p.fecha, variantes: normalizarVariantes(p.nombre_real) })),
    );
  ...
  return rows.map(p => {
    ...
    if (suya) {
      return {
        ...
        motivoTexto: `Le corresponde la línea del ${conFecha(suya, p.fecha)}, de ${eur(suya.importe)}: «${String(suya.concepto || "").slice(0, 70)}»`,
      };
    }
...
async function emparejarIngresosConLarpManager(todasLasFilas, importacionId) {
  ...
  const { porLinea } = repartirUnoAUno(
    ...
    filasCSV.filter(f => !tomados.has(clavePago(f)))
      .map(f => ({ clave: clavePago(f), importe: f.importe, fecha: f.fecha, variantes: f.nombresNormalizados, fila: f })),
  );
