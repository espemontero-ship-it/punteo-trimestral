...
async function listarPagosLarpManagerSinEmparejar() {
  ...
  const { rows: movimientos } = await query(
    `SELECT concepto, importe, fecha FROM movimientos WHERE importe > 0`
  );
  const conceptos = movimientos.map(m => ({
    importe: Number(m.importe), fecha: m.fecha, n: normalizar(m.concepto),
  }));
  ...
  return rows.map(p => {
    ...
    if (porApellido.length > 0) {
      ...
      return {
        ...
        motivoTexto: ordenadas.length === 1
          ? `Encaja con la línea del ${conFecha(ordenadas[0], p.fecha)}. Vuelve a subir el archivo y se cerrará solo.`
          : `Encaja con ${ordenadas.length} líneas del banco, todas de ${eur(p.importe)}: ${ordenadas.map(m => conFecha(m, p.fecha)).join(', ')}. Elige cuál en Vincular.`,
      };
    }
...
async function listarCandidatosParaPago(pagoId) {
  ...
  return rows
    ...
    // Primero los que cuadran de importe, y dentro de cada bloque los más
    // cercanos en fecha al día del pago.
    .sort((a, b) => {
...
async function vincularPagoAMano(pagoId, movimientoId) {
  ...
  return { estadoCambiado: mov.estado !== 'resuelta' };
}
...
async function emparejarIngresosConLarpManager(todasLasFilas, importacionId) {
  ...
  for (const m of movimientos) {
    const cn = normalizar(m.concepto);
    for (const f of filasCSV) {
      if (tomados.has(clavePago(f))) continue;
      if (!tokensCoinciden(f.nombresNormalizados, cn)) continue;
      if (Math.abs(f.importe - Number(m.importe)) > TOLERANCIA) continue;
      parejas.push({ mid: m.id, f, dist: distanciaDias(m.fecha, f.fecha) });
    }
  }
  parejas.sort((a, b) => a.dist - b.dist);
  const asignado = new Map();          // movimiento -> pago
  const pagosUsados = new Set();
  for (const p of parejas) {
    if (asignado.has(p.mid) || pagosUsados.has(clavePago(p.f))) continue;
    asignado.set(p.mid, p.f);
    pagosUsados.add(clavePago(p.f));
  }
