...
// Ingresos que NUNCA son la transferencia de un jugador y por tanto no entran
// en el cruce ni se ofrecen al vincular a mano. La liquidación de remesa es lo
// que ingresa el datáfono: un solo apunte que agrupa muchos cobros, así que no
// se corresponde con el pago de nadie en concreto.
//
// Además ensuciaba de verdad: "OMER" está dentro de "COMERCIOS", así que a
// Omer Shoval Shakked se le proponía en ámbar una liquidación de 161,62 € para
// un pago de 160 €.
...
// Los pagos guardados antes de este cambio tienen la firma vieja (la de la
// fila entera del CSV). Si no se recalculan, el siguiente archivo no
// reconocería ni uno solo y los duplicaría TODOS. Se hace una vez: la
// columna firma_version marca los que ya están al día.
//
// Los pagos que resultan ser el mismo se distinguen con `orden` (0, 1, 2...),
// como se ha hecho siempre con los pagos repetidos de verdad dentro de un
// mismo archivo. Así no se pierde ninguna fila ni se rompe el índice.
...
// Qué hace que dos filas sean el mismo pago: FECHA, NOMBRE E IMPORTE. Nada
// más. El CSV no trae ningún identificador de pago, y la posición en el
// archivo no sirve porque LarpManager pone los nuevos arriba y desplaza el
// resto.
//
// Antes la firma era de la fila ENTERA (todas sus columnas y sus nombres de
// columna). Eso comparaba el formato del archivo, no el pago: el 22/8/2026 un
// export trajo una columna vacía de más al principio y los 32 pagos que ya
// estaban guardados entraron otra vez como nuevos, con sus 32 copias
// enlazadas a líneas del banco. Con estos tres datos, una columna nueva en el
// export no cambia nada.
//
// El nombre se normaliza (sin acentos, sin mayúsculas) para que "María" y
// "Maria" no cuenten como dos pagos distintos.
