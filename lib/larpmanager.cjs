...
// Tratamientos. El banco los escribe delante del nombre y NO identifican a
// nadie: aprender "MISS" para una persona le entrega cualquier otra línea que
// también lo lleve. Pasó en producción -- el pago de Valeria Oliveira se
// quedó el ingreso de Samantha van den Esschert por esto exactamente.
const TRATAMIENTOS = new Set(['MISS', 'MRS', 'MISTER', 'MADAME', 'MADEMOISELLE', 'HERR', 'FRAU', 'DON', 'DONA', 'SENOR', 'SENORA']);

// Palabras que son el nombre de OTRO jugador. Nunca se aprenden: un nombre
// ajeno dentro de una línea no dice cómo llama el banco a esta persona, dice
// que esa línea es de la otra. Así se aprendió "SAMANTHA" como alias de
// Valeria Oliveira.
async function nombresDeOtros(nombreJugador) {
  const suyo = normalizarVariantes(nombreJugador)[0];
  const { rows } = await query(`SELECT DISTINCT nombre_real FROM larpmanager_pagos`);
  const ajenas = new Set();
  for (const r of rows) {
    const variantes = normalizarVariantes(r.nombre_real);
    if (variantes.includes(suyo)) continue;
    for (const v of variantes) for (const w of v.split(' ')) if (w.length >= 3) ajenas.add(w);
  }
  // Lo que también está en su propio nombre no es de nadie más.
  for (const v of normalizarVariantes(nombreJugador)) for (const w of v.split(' ')) ajenas.delete(w);
  return ajenas;
}

async function aprenderComoLlamaElBanco(nombreJugador, concepto, ordenante) {
  await asegurarTablaAlias();
  const prohibidas = await nombresDeOtros(nombreJugador);
  const vale = w => !TRATAMIENTOS.has(w) && !prohibidas.has(w);

  // Si la línea trae la columna del ordenante, se aprende de ahí y punto: lo
  // que hay en esa columna ES el nombre de quien paga, sin el "ABONO POR
  // TRANSFERENCIA A SU FAVOR..." delante ni la referencia detrás. No hace
  // falta ningún umbral ni adivinar qué palabra es el nombre.
  if (ordenante && ordenante.trim()) {
    const suyasYa = new Set(normalizarVariantes(nombreJugador).flatMap(v => v.split(' ')));
    const delOrdenante = [...new Set(normalizar(ordenante).split(' ').filter(t => t.length >= 3))]
      .filter(w => !suyasYa.has(w) && vale(w));
    if (delOrdenante.length > 0) {
      const nombreN = normalizarVariantes(nombreJugador)[0];
      for (const w of delOrdenante) {
        await query(`INSERT INTO larpmanager_alias (nombre, palabra) VALUES ($1,$2) ON CONFLICT DO NOTHING`, [nombreN, w]);
      }
      return delOrdenante.length;
    }
    return 0;
  }

  // Sin esa columna (movimientos importados antes de guardarla, u otro banco)
  // se cae al método de antes: mirar toda la línea y quedarse con las
  // palabras poco frecuentes. Es peor y por eso necesita el umbral.
  ...
  const utiles = palabras.filter(w => !suyas.has(w) && vale(w) && (frecuencia.get(w) || 0) <= APARICIONES_MAX);
...
async function listarPagosLarpManagerSinEmparejar() {
  ...
  // De quién es la línea que le falta a este pago. Sin esto, "ya justifican
  // otros pagos suyos" obligaba a adivinar cuáles.
  const quienLaTiene = (m, p) => {
    if (!m.ocupadaPor) return '';
    const cuando = m.ocupadaPor.fecha ? ` del ${dia(m.ocupadaPor.fecha)}` : '';
    return m.ocupadaPor.nombre === p.nombre_real
      ? `, ya la justifica su pago${cuando}`
      : `, ya la justifica un pago de ${m.ocupadaPor.nombre}`;
  };
  ...
  return rows.map(p => {
    ...
    if (p.movimiento_id) {
      return {
        ...p, motivo: 'emparejado',
        motivoTexto: `Emparejado con el movimiento del ${conFecha({ fecha: p.movimiento_fecha }, p.fecha)}, de ${eur(p.movimiento_importe)}: «${p.movimiento_concepto || ''}»`,
      };
    }
    // A un pago que ya has contestado no se le busca explicación: no está
    // esperando nada.
    if (p.estado === 'ignorada') {
      return { ...p, motivo: 'ignorada', motivoTexto: 'Ignorado a mano: de este pago no se espera ningún ingreso.' };
    }
    if (p.estado === 'resuelta') {
      return { ...p, motivo: 'resuelta_a_mano', motivoTexto: 'Dado por bueno a mano, sin movimiento.' };
    }
    ...
    if (suya) {
      return {
        ...
        motivoTexto: `Mismo apellido y mismo importe, con ${Math.abs(desfase(suya.fecha, p.fecha) ?? 0)} día(s) de diferencia. Compruébalo antes de aceptar.`,
      };
    }
    ...
    if (porApellido.length > 0) {
      ...
      if (cuadran.length === 0) {
        ...
        return {
          ...
          motivoTexto: `El apellido sí aparece en el banco (${otros.map(m => `${eur(m.importe)} el ${conFecha(m, p.fecha)}${quienLaTiene(m, p)}`).join('; ')}), pero ninguno es de ${eur(p.importe)}.`,
        };
      }
      // Su nombre y su importe encajan, pero las líneas ya están dadas: o se
      // las han quedado otros pagos suyos más cercanos en fecha, o son de
      // otra persona que el banco escribe parecido.
      const ordenadas = [...cuadran].sort(porCercania(p.fecha));
      return {
        ...p, motivo: 'sin_linea_libre',
        motivoTexto: `Hay ${ordenadas.length} movimiento(s) de ${eur(p.importe)} a su nombre (${ordenadas.map(m => `${conFecha(m, p.fecha)}${quienLaTiene(m, p)}`).join('; ')}). O falta esta transferencia por llegar, o alguno de esos pagos está de más.`,
      };
    }
    ...
    if (parciales.length > 0) {
      ...
      return {
        ...
        motivoTexto: `Solo coincide parte del nombre, no el apellido, en ${parciales.length} movimiento(s): ${ordenadas.slice(0, 4).map(m => `${eur(m.importe)} el ${conFecha(m, p.fecha)}${quienLaTiene(m, p)}`).join('; ')}. Puede ser otra persona: hay que mirarlo a mano.`,
      };
    }

    return {
      ...p, motivo: 'no_esta',
      motivoTexto: 'El nombre no aparece en ningún movimiento del banco. O la transferencia no ha llegado, o falta por subir el excel de esas fechas.',
    };
  });
}
...
module.exports = {
  ...
  historialDeJugador, cambiarEstadoPago,
