...
async function listarPagosCandidatosParaMovimiento(movimientoId) {
  ...
  // Solo pagos que están esperando: uno ya enlazado tiene su movimiento, y uno
  // ignorado o dado por bueno a mano ya está contestado.
  const { rows } = await query(
    `SELECT id, nombre_real, evento, importe, fecha FROM larpmanager_pagos
     WHERE movimiento_id IS NULL AND entra_en_cruce AND estado = 'pendiente'`
  );
  ...
  return rows.map(p => {
    ...
    return {
      ...
      importe: Number(p.importe), fecha: p.fecha,
