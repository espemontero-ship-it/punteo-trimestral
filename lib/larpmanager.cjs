...
async function listarPagosLarpManagerSinEmparejar() {
  ...
  const conceptos = movimientos.map(m => ({
    id: m.id, importe: Number(m.importe), fecha: m.fecha, n: normalizar(m.concepto),
  }));
  ...
  return rows.map(p => {
    ...
    if (suya) {
      return {
        ...
        motivoTexto: `Le corresponde la línea del ${conFecha(suya, p.fecha)}, de ${eur(suya.importe)}. Vuelve a subir el archivo y se cerrará solo.`,
      };
    }
