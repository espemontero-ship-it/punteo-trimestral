  ...
  return rows.map(p => {
    ...
    if (suya) {
      return {
        ...
        motivoTexto: `Le corresponde el movimiento del ${conFecha(suya, p.fecha)}, de ${eur(suya.importe)}: «${suya.concepto || ''}»`,
      };
    }

    // Un movimiento que ya justifica el pago de OTRA persona no es una pista:
    // no puede ser suyo, y nombrarlo hace leer que la app se lo atribuye. Con
    // dos apellidos iguales pasa constantemente -- a Paloma Vargas se le
    // listaban las transferencias de Sara Albaladejo Vargas. Los ocupados por
    // sus PROPIOS pagos sí se nombran: ahí lo útil es saber cuál se lo llevó.
    const deOtro = m => m.ocupadaPor && m.ocupadaPor.nombre !== p.nombre_real;
    const mencionables = ms => ms.filter(m => !deOtro(m));
    const soloDeOtros = ms => ms.length > 0 && mencionables(ms).length === 0;

    if (porApellido.length > 0) {
      const cuadran = porApellido.filter(m => Math.abs(m.importe - Number(p.importe)) <= TOLERANCIA);
      if (cuadran.length === 0) {
        if (soloDeOtros(porApellido)) {
          return {
            ...p, motivo: 'importe_no_cuadra',
            motivoTexto: `El apellido aparece en el banco, pero en movimientos que ya justifican pagos de otra persona. Ninguno libre es de ${eur(p.importe)}.`,
          };
        }
        const otros = mencionables(porApellido).sort(porCercania(p.fecha));
        return {
      ...
      if (soloDeOtros(cuadran)) {
        return {
          ...p, motivo: 'sin_linea_libre',
          motivoTexto: `Hay movimientos de ${eur(p.importe)} con ese apellido, pero ya justifican pagos de otra persona. O falta esta transferencia por llegar, o el apellido lo comparten dos jugadores.`,
        };
      }
      const ordenadas = mencionables(cuadran).sort(porCercania(p.fecha));
      return {
        ...p, motivo: 'sin_linea_libre',
    ...
    if (parciales.length > 0 && !soloDeOtros(parciales)) {
      const ordenadas = mencionables(parciales).sort(porCercania(p.fecha));
      return {
        ...p, motivo: 'nombre_parcial', sugerencia: proponerDudosa(parciales, p),
        motivoTexto: `Solo coincide parte del nombre, no el apellido, en ${ordenadas.length} movimiento(s): ${ordenadas.slice(0, 4).map(m => `${eur(m.importe)} el ${conFecha(m, p.fecha)}${quienLaTiene(m, p)}`).join('; ')}. Puede ser otra persona: hay que mirarlo a mano.`,
      };
    }
...
async function listarPagosCandidatosParaMovimiento(movimientoId) {
  ...
  // Salen también los ignorados y los dados por buenos a mano, aunque no se
  // puedan enlazar. Filtrarlos los hacía invisibles justo cuando hacen falta:
  // si el ingreso que tienes delante es de alguien a quien diste por ignorado,
  // la única forma de darte cuenta es verlo en esta lista. La pantalla los
  // enseña con su estado y sin botón.
  const { rows } = await query(
    `SELECT id, nombre_real, evento, importe, fecha, estado FROM larpmanager_pagos
