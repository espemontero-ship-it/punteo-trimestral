  ...
  return rows.map(p => {
    ...
    if (porApellido.length > 0) {
      ...
      if (cuadran.length === 0) {
        ...
        return {
          ...
          motivoTexto: `El apellido sí aparece en el banco (${otros.map(m => `${eur(m.importe)} el ${conFecha(m, p.fecha)}`).join('; ')}), pero ninguno es de ${eur(p.importe)}.`,
        };
      }
    ...
    if (parciales.length > 0) {
      ...
      return {
        ...
        motivoTexto: `Solo coincide parte del nombre, no el apellido, en ${parciales.length} movimiento(s): ${ordenadas.slice(0, 4).map(m => `${eur(m.importe)} el ${conFecha(m, p.fecha)}`).join('; ')}. Puede ser otra persona: hay que mirarlo a mano.`,
      };
    }
