...
// nada que solo viva en el navegador justo después de subir el CSV (la
// misma lección que ya costó un bug real con el botón de confirmar).
...
async function asegurarTablaPagosLarpManager() {
  ...
  await query(`
    CREATE TABLE IF NOT EXISTS larpmanager_pagos (
      ...
      UNIQUE (trimestre_id, nombre_real, evento, importe, fecha)
    )
  `);
...
async function guardarPagosLarpManager(trimestreId, filasCSV) {
  ...
  filasCSV.forEach((f, i) => {
    ...
    valores.push(trimestreId, f.nombreReal, f.evento, f.importe, f.fecha ? f.fecha.toISOString().slice(0, 10) : null);
  });

  await query(
    `INSERT INTO larpmanager_pagos (trimestre_id, nombre_real, evento, importe, fecha)
     VALUES ${placeholders.join(', ')}
     ON CONFLICT (trimestre_id, nombre_real, evento, importe, fecha) DO NOTHING`,
    valores
  );
}

// Todos los pagos de LarpManager de este trimestre que ningún movimiento
// del banco ha reclamado todavía -- el hueco que antes no se podía ver: si
// LarpManager dice que alguien pagó pero esa transferencia nunca aparece
// emparejada, aquí se queda listada para revisar a mano.
async function listarPagosLarpManagerSinEmparejar(trimestreId) {
  await asegurarTablaPagosLarpManager();
  const { rows } = await query(
    `SELECT id, nombre_real, evento, importe, fecha FROM larpmanager_pagos
     WHERE trimestre_id = $1 AND movimiento_id IS NULL
...
async function resolverPagoLarpManager(movimientoId, candidato) {
  await asegurarTablaPagosLarpManager();
  const { rows } = await query(`SELECT trimestre_id, proyecto_id FROM movimientos WHERE id = $1`, [movimientoId]);
  if (rows.length === 0) throw new Error('Movimiento no encontrado.');
  const { trimestre_id: trimestreId, proyecto_id: proyectoActual } = rows[0];
  ...
  await query(
    `UPDATE larpmanager_pagos SET movimiento_id = $1
     WHERE trimestre_id = $2 AND nombre_real = $3 AND evento = $4 AND importe = $5 AND fecha = $6`,
    [movimientoId, trimestreId, candidato.nombreReal, candidato.evento, candidato.importe, candidato.fecha]
  );
}
...
async function emparejarIngresosConLarpManager(trimestreId, filasCSV) {
  ...
  await guardarPagosLarpManager(trimestreId, filasCSV);
