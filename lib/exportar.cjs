...
async function generarPaqueteTrimestre(trimestreId) {
  ...
  const listo = new Promise((resolve, reject) => {
    ...
    archive.on('error', err => reject(new Error(`[comprimir zip] ${err.message}`)));
  });

  await etapa('añadir excel final al zip', () => archive.append(excelBuffer, { name: `punteo-${trimestreId}.xlsx` }));
  ...
  await etapa('finalizar zip', () => archive.finalize());
