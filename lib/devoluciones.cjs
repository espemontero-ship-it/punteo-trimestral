...
// Para revisar/exportar antes de cerrar el trimestre (pestaña "Devoluciones"
// del excel final, y declaración de IVA del trimestre).
async function listarDevolucionesTrimestre(trimestreId) {
  await asegurarColumnasDevolucion();
  const { rows } = await query(
    `SELECT m.id, m.fecha, m.importe, m.jugador_larpmanager, m.nota_final, p.nombre AS proyecto
     ...
     WHERE m.trimestre_id = $1 AND m.es_devolucion = true
     ORDER BY m.fecha`,
    [trimestreId]
  );
  return rows;
...
// todas sus devoluciones aunque estén repartidas entre varios trimestres.
async function listarDevolucionesProyecto(proyectoId) {
  await asegurarColumnasDevolucion();
  const { rows } = await query(
    `SELECT m.id, m.trimestre_id, m.fecha, m.importe, m.jugador_larpmanager, m.nota_final
     FROM movimientos m
     WHERE m.proyecto_id = $1 AND m.es_devolucion = true
     ORDER BY m.trimestre_id, m.fecha`,
    [proyectoId]
  );
  return rows;
}
...
module.exports = {
  ...
  marcarDevolucion, listarDevolucionesTrimestre, listarDevolucionesProyecto,
};
