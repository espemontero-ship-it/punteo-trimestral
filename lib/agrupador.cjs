...
async function construirProveedores(trimestreId) {
  ...
  const { rows: movimientosBrutos } = await query(
    `SELECT m.id, m.hoja, m.fila, m.fecha, m.concepto, m.importe, m.clave, m.estado, m.nota_final,
            m.datos_originales, m.proyecto_id, p.nombre AS proyecto_nombre,
            COALESCE(
...
// Cambia el estado de una única línea entre "pendiente" (sin_resolver) y
// "pedida" (pedida_pendiente) — el desplegable de Estado en la tabla.
async function marcarLineaPendiente(movimientoId, pedida) {
  const nuevoEstado = pedida ? 'pedida_pendiente' : 'sin_resolver';
  await query(
    `UPDATE movimientos SET estado = $1 WHERE id = $2 AND estado != 'resuelta'`,
    [nuevoEstado, movimientoId]
  );
}

module.exports = { construirProveedores, confirmarGrupo, confirmarLinea, marcarGrupoPendiente, marcarLineaPendiente };
