...
const { asegurarColumnasMotivo } = require('./facturaMatcher.cjs');
...
async function construirProveedores(desde, hasta) {
  ...
  // La consulta de abajo pregunta por larpmanager_pagos, así que la tabla
  // tiene que existir antes. Sin esto, una base de datos donde todavía no se
  // haya subido ningún CSV se quedaría sin pestaña Movimientos entera --
  // exactamente el fallo que tuvo "Archivos subidos" el 2026-08-16.
  await asegurarTablaPagosLarpManager();
  await asegurarColumnasDevolucion();
  // Mismo motivo: la consulta lee `facturas.motivo_tipo` y `motivo_candidatos`
  // para traerse las combinaciones sugeridas, y esas columnas las crea el
  // emparejador de facturas.
  ...
  const { rows: movimientosBrutos } = await query(
    `SELECT m.id, m.hoja, m.fila, m.fecha, m.concepto, m.importe, m.clave, m.estado, m.nota_final,
            ...
            -- Los pagos de LarpManager enlazados con esta línea, para poder
            -- deshacer el vínculo desde la propia fila (el ✎ de la columna
            -- LarpManager). El texto de datos_originales.larpmanager no vale
            -- para esto: dice lo que encontró el cruce el día que se subió el
            -- CSV ("no encontrada", "3 coincidencias posibles"), no si al
            -- final quedó enlazada ni con qué pago.
            COALESCE(
            ...
            -- Combinaciones de facturas que apuntan a ESTA línea. La
            -- sugerencia se calculaba ya, pero solo se veía en la pestaña
            -- Facturas: desde Movimientos, una línea pendiente cuyo importe
            -- ya estaba explicado por dos facturas seguía pareciendo que no
            -- tenía nada. El id de la línea viaja dentro de motivo_candidatos
            -- (ver candidatosParaGuardar en facturaMatcher.cjs).
            COALESCE(
              (SELECT jsonb_agg(jsonb_build_object(
                        'facturaId', f.id, 'numero', f.numero, 'concepto', f.concepto,
                        'detalle', f.motivo_detalle,
                        -- Si la suma no cuadra al céntimo, cuánto se desvía.
                        -- Va también aquí porque el aviso tiene que salir en
                        -- la píldora de Movimientos, no solo en Facturas.
                        'exacto', f.motivo_candidatos->'exacto',
                        'diferencia', f.motivo_candidatos->'diferencia',
                        -- Hay dos formatos guardados: el de ahora, con una
                        -- lista otrasFacturas, y el viejo, con una sola
                        -- otraFacturaId. Una fila vieja que no se haya
                        -- recalculado desde entonces sigue en el formato
                        -- viejo (en desarrollo hay una), y leer solo el nuevo
                        -- haría que la línea se cerrara con una factura de
                        -- las dos -- justo el error que la sugerencia existe
                        -- para evitar.
                        'otras', COALESCE(
                          f.motivo_candidatos->'otrasFacturas',
                          CASE WHEN (f.motivo_candidatos->>'otraFacturaId') IS NOT NULL
                            THEN jsonb_build_array(jsonb_build_object(
                                   'id', f.motivo_candidatos->'otraFacturaId',
                                   'numero', f.motivo_candidatos->'otraFacturaNumero'))
                            ELSE '[]'::jsonb END)) ORDER BY f.numero)
               FROM facturas f
  ...
  const movimientos = movimientosBrutos.map(m => {
    ...
    // La ✕ de una combinación de facturas se guarda como cualquier otro
    // rechazo, con las facturas concretas dentro del valor: decir que no a
    // "17 + 16" no es decir que no a cualquier combinación futura de esta
    // línea.
    const combos_factura = (m.combos_factura || []).filter(c => {
      const ids = [c.facturaId, ...(c.otras || []).map(o => o.id)].join(',');
      return !rechazada('combo', ids);
    });

    return { ...m, proyecto_sugerido, proveedor_sugerido, probable_devolucion, jugador_sugerido, combos_factura };
  });
