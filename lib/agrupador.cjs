...
async function construirProveedores(trimestreId) {
  ...
  const resultado = [...grupos.values()].map(g => {
    ...
    const sinResolver = total - resueltas - pedidaPendiente - facturaFutura;
    return { ...g, total, resueltas, pedidaPendiente, facturaFutura, sinResolver, completo: sinResolver === 0 };
  });
...
async function confirmarGrupo(trimestreId, hoja, clave, nota) {
  const { rows } = await query(
    `UPDATE movimientos SET estado = 'resuelta', nota_final = $1
     ...
     RETURNING id`,
    [nota || null, trimestreId, hoja, clave]
  );
  if (nota) {
...
// Cambia el estado de una única línea a "pendiente" (sin_resolver), "pedida"
// (pedida_pendiente) o "factura futura" (factura_futura) — el desplegable de
// Estado en la tabla. A propósito no exige que la línea no estuviera ya
// resuelta: sirve también para deshacer una línea resuelta por error,
// volviéndola a pendiente/pedida/factura futura.
async function marcarLineaEstado(movimientoId, estado) {
  const nuevoEstado = estado === 'pedida' ? 'pedida_pendiente' : estado === 'factura_futura' ? 'factura_futura' : 'sin_resolver';
...
// que ha pasado el servicio, ej. DoYouSpain/Iberia) todavía sin recuperar de
// cualquier trimestre, no solo del actual. Mismo criterio que
// listarDevolucionesProyecto en lib/devoluciones.cjs.
async function listarFacturasFuturasProyecto(proyectoId) {
  const { rows } = await query(
    `SELECT id, trimestre_id, fecha, importe, proveedor, concepto
     FROM movimientos
     WHERE proyecto_id = $1 AND estado = 'factura_futura'
     ORDER BY trimestre_id, fecha`,
    [proyectoId]
  );
  return rows;
}
