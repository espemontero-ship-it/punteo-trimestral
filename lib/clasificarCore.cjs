...
function aplicarClasificacion(wb, memoria) {
  ...
  for (const cfg of sheetsConfig) {
    ...
    resumen.push({ hoja: cfg.nombre, nuevas, fijas, mixtas, clavesNuevas: [...clavesNuevas] });
  }
...
function escribirResumenClasificacion(wb, resumen) {
  ...
  ws.addRow(['Hoja', 'Nuevas (sin historial)', 'Sugerencia fija', 'Histórico mixto']).font = { bold: true };
  for (const r of resumen) ws.addRow([r.hoja, r.nuevas, r.fijas, r.mixtas]);
  ws.addRow([]);
