    ...
    facturas: {
      ...
      items: {
        ...
        properties: {
          importe: {
            type: 'number',
            description: 'Importe TOTAL a pagar de esta factura, IVA incluido, en euros y positivo (ej. 45.00).',
          },
          fecha: {
            type: ['string', 'null'],
...
async function leerFacturaConIA(buffer, esPdf, nombreOriginal) {
  ...
  try {
    ...
    const response = await client.messages.parse({
      ...
      messages: [{
        ...
        content: [
          ...
          {
            type: 'text',
            text: 'Este documento es una factura o recibo de gastos de una asociación. Para cada factura que contenga, extrae el importe TOTAL a pagar (IVA incluido), la fecha de emisión y el nombre del proveedor. Casi siempre habrá una sola: devuelve varias únicamente si el documento contiene de verdad facturas independientes con totales distintos. No devuelvas la base imponible ni el IVA como si fueran facturas aparte. Si no puedes leer el documento o no es una factura, indica legible=false y devuelve la lista vacía.',
          },
        ],
      }],
    });
    ...
    const facturas = (datos && datos.facturas ? datos.facturas : [])
      .filter(f => typeof f.importe === 'number' && !isNaN(f.importe) && f.importe > 0);
    if (!datos || !datos.legible || facturas.length === 0) {
