...
async function listarFacturasDeLote(loteId) {
  const { rows } = await query(
    `SELECT id, numero, nombre_original, ruta_blob, concepto, importe_declarado, estado_revision, motivo_rechazo, es_imagen, creado_en
...
async function subirFacturaLote({ loteId, trimestreId, rutaBlob, nombreOriginal, concepto, importe, esImagen }) {
  ...
  const { rows } = await query(
    `INSERT INTO facturas (trimestre_id, lote_id, ruta_blob, nombre_original, numero, concepto,
                            importe_declarado, estado_revision, es_imagen, estado)
     VALUES ($1,$2,$3,$4,$5,$6,$7,'subida',$8,'revisar')
     RETURNING id, numero`,
    [trimestreId, loteId, rutaBlob, nombreOriginal, numero, concepto || null, importe || null, !!esImagen]
  );
  return rows[0];
}

async function actualizarFactura(facturaId, { concepto, importe, estadoRevision, motivoRechazo }) {
  await query(
    `UPDATE facturas SET
       concepto = COALESCE($2, concepto),
       importe_declarado = COALESCE($3, importe_declarado),
       estado_revision = COALESCE($4, estado_revision),
       motivo_rechazo = $5
     WHERE id = $1`,
    [facturaId, concepto ?? null, importe ?? null, estadoRevision ?? null, motivoRechazo ?? null]
  );
}
...
module.exports = {
  crearLote, listarLotesPorTrimestre, listarLotesPorColaborador, obtenerLote,
