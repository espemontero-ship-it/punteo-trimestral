...
// contraseña.
async function crearColaboradorYLote(nombre, usuario, proyectoId) {
  ...
  const loteId = await crearLote(trimestreActual(), colaborador.id, proyectoId);
...
async function listarFacturasPendientesPorProyecto(proyectoId) {
  const { rows } = await query(
    `SELECT f.id, f.concepto, f.importe_declarado, f.estado_revision, f.creado_en,
...
module.exports = {
  crearLote, crearColaboradorYLote, trimestreActual, listarLotesPorTrimestre, listarTodosLosLotes, listarLotesPorColaborador, obtenerLote,
