...
const { siguienteNumero, facturaConMismaHuella } = require('./facturaMatcher.cjs');
...
async function subirFacturaLote({ loteId, trimestreId, rutaBlob, nombreOriginal, concepto, importe, fecha, esImagen }) {
  // La huella del archivo, igual que en el flujo principal: sha256 de lo que
  // hay dentro. Aquí el archivo no se lee para sacar importes (los declara el
  // colaborador), pero sí se baja para calcularla -- bajarlo no cuesta dinero,
  // y sin huella esta factura no se podría reconocer si se sube dos veces.
  const buffer = await descargarBlob(rutaBlob);
  const huella = crypto.createHash('sha256').update(buffer).digest('hex');
  const yaSubida = await facturaConMismaHuella(huella);
  if (yaSubida) {
  ...
  // Misma numeración que las facturas del flujo principal (ver siguienteNumero).
  const numero = await siguienteNumero();
  const { rows } = await query(
    `INSERT INTO facturas (trimestre_id, lote_id, ruta_blob, nombre_original, numero, concepto,
                            importe_declarado, fechas, estado_revision, es_imagen, estado, huella)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'subida',$9,'revisar',$10)
     RETURNING id, numero`,
    [trimestreId, loteId, rutaBlob, nombreOriginal, numero, concepto || null, importe || null, fecha ? [fecha] : [], !!esImagen, huella]
  );
  return rows[0];
}
...
async function listarFacturasPendientesPorProyecto(proyectoId) {
  ...
  return rows.map(r => ({
    ...r,
    importe_declarado: r.importe_declarado ?? importeDeFactura(r),
  }));
}
