...
// El importe de las facturas de unos lotes, sacado con la MISMA funcion que
// usa todo lo demas (importeFactura.cjs). En SQL no se puede sumar: el
// importe de una factura es la suma de sus totales, y eso vive en JS.
// El importe declarado a mano solo queda de red para las facturas viejas,
// de cuando el colaborador escribia el importe en vez de leerse del documento.
async function importesPorLote(loteIds) {
  const ids = loteIds.filter(Boolean);
  if (ids.length === 0) return new Map();
  const { rows } = await query(
    `SELECT lote_id, totales, importe_declarado, estado_revision FROM facturas WHERE lote_id = ANY($1::bigint[])`,
    [ids]
  );
  const mapa = new Map();
  for (const f of rows) {
    ...
    mapa.get(clave).push({
      importe: leido === null || leido === undefined ? Number(f.importe_declarado || 0) : leido,
      estado: f.estado_revision,
    });
  }
  return mapa;
}
...
async function listarFacturasDeLote(loteId) {
  ...
  const { rows } = await query(
    `SELECT id, numero, nombre_original, ruta_blob, concepto, importe_declarado, totales, proveedor, importe_a_mano,
...
// Recibe la lectura ya hecha (`analisis`), igual que procesarFacturaSubida:
// quien descarga el archivo y llama a la IA es la ruta, no esto. Asi esta
// funcion se puede probar sin bajar nada ni gastar en IA.
//
// Una factura de colaborador se guarda como cualquier otra --mismo numero,
// mismos campos leidos, SIN trimestre-- y lo unico que la distingue es que
// vive en un lote: la paga el, asi que no se cruza con el banco.
...
// El colaborador corrige lo suyo MIENTRAS ESTE SIN REVISAR. En cuanto se
// acepta, se rechaza o se cierra, ya no puede tocarla: lo aceptado cuenta como
...
// Retirar la borra DE VERDAD, con su archivo. Si solo se marcara, su huella
// seguiria ocupada y no podria volver a subir el mismo archivo corregido --
// que es justo para lo que sirve retirar.
async function retirarFacturaColaborador(colaboradorId, facturaId) {
  ...
  try { await eliminarBlob(f.ruta_blob); } catch { /* el archivo ya no esta o no se deja borrar */ }
}
...
// Los totales salen del importe LEIDO de cada factura, con la misma funcion
// que usa todo lo demas (importeFactura.cjs). Antes se sumaba el importe que
// declaraba el colaborador a mano.
async function calcularTotales(loteId) {
  const { rows: facturas } = await query(
    `SELECT totales, importe_declarado, estado_revision FROM facturas
  ...
  const suma = estados => Math.round(facturas
    ...
    .reduce((acc, f) => {
      const leido = importeDeFactura(f);
      return acc + (leido === null || leido === undefined ? Number(f.importe_declarado || 0) : leido);
    }, 0) * 100) / 100;
...
async function listarFacturasPendientesPorProyecto(proyectoId) {
  const { rows } = await query(
    `SELECT f.id, f.concepto, f.importe_declarado, f.totales, f.estado_revision, f.creado_en, 'colaborador' AS pagado_por,
     ...
     UNION ALL
     SELECT f.id, f.concepto, f.importe_declarado, f.totales, f.estado_revision, f.creado_en, 'nol' AS pagado_por,
  ...
  // El importe sale del unico sitio que lo calcula, igual que en Facturas y en
  // el cruce. Antes aqui se cogia el primer numero de la lista.
  return rows.map(r => ({
    ...r,
    importe_declarado: importeDeFactura(r) ?? r.importe_declarado,
  }));
}
