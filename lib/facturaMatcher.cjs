...
// guardado), esto vuelve a descargar y leer el PDF de cada factura sin
// resolver -- para cuando se corrige el regex de lectura o un bug de
// matching y hay facturas que se quedaron mal por eso, no por falta de
// movimientos. Más lento (descarga cada archivo), así que es una acción a
// mano, no automática.
async function reprocesarFacturas(trimestreId) {
  const { rows: pendientes } = await query(
    `SELECT id, ruta_blob FROM facturas
     WHERE trimestre_id = $1 AND estado IN ('sin_match', 'revisar') AND es_imagen = false`,
    [trimestreId]
  );
  let resueltas = 0;
  for (const f of pendientes) {
    try {
      ...
      const { rows } = await query(
        ...
        [analisis.importes, analisis.totales, analisis.fechas.map(d => d.toISOString().slice(0, 10)), f.id]
      );
      ...
      // Un archivo suelto que falle (borrado del blob, pdf corrupto...) no
      // debe frenar el resto de la lista.
    }
  }
  return { revisadas: pendientes.length, resueltas };
}
...
module.exports = { procesarFacturaSubida, confirmarMatch, confirmarImporteManual, reintentarPendientes, reprocesarFacturas, siguienteNumero };
