...
// guarda lo que pone.
// Rellena también la huella del archivo en las facturas que se subieron antes
// de que se guardara: si no, el aviso de "este archivo ya está subido" solo
// funcionaría contra las facturas nuevas.
async function guardarTextosFaltantes(limite = 200) {
  await asegurarColumnasMotivo();
  const { rows } = await query(
    `SELECT id, numero, ruta_blob, nombre_original, texto IS NULL AS falta_texto FROM facturas
     WHERE (texto IS NULL OR huella IS NULL) AND ruta_blob IS NOT NULL
     ORDER BY numero LIMIT $1`, [limite]
  );
  const resultado = { total: rows.length, guardadas: 0, huellas: 0, sinTexto: 0, fallos: [] };
  for (const f of rows) {
    try {
      const buffer = await descargarBlob(f.ruta_blob);
      await query(`UPDATE facturas SET huella = $2 WHERE id = $1`,
        [f.id, crypto.createHash('sha256').update(buffer).digest('hex')]);
      resultado.huellas++;
      if (!f.falta_texto) continue;
