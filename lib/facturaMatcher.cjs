...
async function emparejamientoDe(facturaId) {
  ...
  const { rows: facturas } = await query(
    `SELECT f.id, f.numero, f.totales, f.importes FROM movimiento_facturas mf
     JOIN facturas f ON f.id = mf.factura_id
     WHERE mf.movimiento_id = $1 ORDER BY f.numero`, [mov[0].id]
  );
  const suma = facturas.reduce((acc, f) => acc + (montoCaracteristico(f.totales, f.importes) || 0), 0);
...
async function facturaConMismaHuella(huella, excluirId = 0) {
  ...
  const { rows } = await query(
    `SELECT id, numero, nombre_original FROM facturas
...
async function procesarFacturaSubida({ hoja, clave, rutaBlob, nombreOriginal, concepto, analisis, subidoPor, proyectoId }) {
  ...
  const insert = await query(
    `INSERT INTO facturas (proveedor_clave, ruta_blob, nombre_original, numero,
                            ...
                            texto, lectura_regex, leido_con_ia, proveedor, huella)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,'sin_match',$10,$11,$12,$13,$14,$15,$16)
     RETURNING *`,
    [
      ...
      analisis.texto || null,
      analisis.lecturaRegex ? JSON.stringify(analisis.lecturaRegex) : null,
      analisis.leidoConIA ?? null,
  ...
  // Si la IA no ha podido leer, se dice: los importes vienen del lector de
  // texto, que es peor, y conviene saberlo antes de fiarse del resultado.
  if (analisis.leidoConIA === false && analisis.motivoIA) {
    resultado.detalle = `Leída sin IA (${analisis.motivoIA}): los importes salen del lector de texto. ${resultado.detalle || ''}`.trim();
  }
  // Se avisa DESPUÉS de guardarla, no en vez de guardarla: la subida no se
  // bloquea nunca (puede ser el mismo archivo subido a propósito), solo se
  // dice que ya estaba, con qué número, para poder borrar la que sobre.
  if (yaSubida) {
    resultado.duplicada = { numero: yaSubida.numero, nombre: yaSubida.nombre_original };
    resultado.detalle = `Este mismo archivo ya está subido como factura #${yaSubida.numero} (${yaSubida.nombre_original}). ${resultado.detalle || ''}`.trim();
  }
...
// Facturas a las que les falta la huella del archivo o el proveedor. Son las
// que se subieron antes de que existieran esas dos columnas: sin huella, el
// aviso de "este archivo ya está subido" no las ve, y el proveedor sale vacío
// en la tabla.
async function listarFacturasIncompletas() {
  await asegurarColumnasMotivo();
  const { rows } = await query(
    `SELECT id FROM facturas
     WHERE ruta_blob IS NOT NULL AND (huella IS NULL OR proveedor IS NULL)
     ORDER BY numero`
  );
  return rows.map(r => r.id);
}

// Rellena esas dos columnas en UNA factura. Deliberadamente NO toca importes,
// fechas ni estado, y no vuelve a cruzar nada: hay facturas ya emparejadas y
// confirmadas a mano, y esto no puede deshacer ese trabajo. Solo escribe lo
// que faltaba.
//
// La huella sale del archivo (gratis). El proveedor hay que leerlo, y eso son
// unos céntimos por factura, así que solo se pide si de verdad falta.
async function completarDatosFactura(facturaId) {
  const { rows } = await query(
    `SELECT numero, ruta_blob, nombre_original, huella, proveedor FROM facturas WHERE id = $1`, [facturaId]
  );
  if (rows.length === 0) throw new Error('Factura no encontrada.');
  const f = rows[0];
  const resultado = { numero: f.numero, huella: false, proveedor: null, motivoIA: null };

  const buffer = await descargarBlob(f.ruta_blob);
  if (!f.huella) {
    await query(`UPDATE facturas SET huella = $2 WHERE id = $1`,
      [facturaId, crypto.createHash('sha256').update(buffer).digest('hex')]);
    resultado.huella = true;
  }

  if (f.proveedor === null) {
    const ia = await leerFacturaConIA(buffer, /\.pdf$/i.test(f.nombre_original || ''), f.nombre_original);
    const proveedor = ia.ok ? (ia.facturas.map(x => x.proveedor).filter(Boolean)[0] || null) : null;
    if (proveedor) {
      await query(`UPDATE facturas SET proveedor = $2 WHERE id = $1`, [facturaId, proveedor]);
      resultado.proveedor = proveedor;
    } else if (ia.ok) {
      // Leyó el documento y ahí no pone quién lo emite. Se guarda vacío (que
      // en la tabla se ve igual que si no hubiera nada) para no volver a
      // pagar una lectura por esta factura cada vez que se abre la pantalla.
      await query(`UPDATE facturas SET proveedor = '' WHERE id = $1`, [facturaId]);
    } else {
      resultado.motivoIA = porQueNoLeyoLaIA(ia.error);
    }
  }
  return resultado;
}

// La lectura de una factura al subirla. Dos pasadas, y las dos importan:
//
//   1. El regex de siempre, SIEMPRE. Además de los números saca el texto del
//      PDF, que se guarda para poder revisar después por qué se eligió un
//      importe y no otro.
//   2. La IA. Si lee, sus importes mandan sobre los del regex: ve el
//      documento en vez de buscar números junto a la palabra "total", así que
//      no confunde la base imponible con el total ni se inventa facturas
//      sumando trozos. Y sabe decir cuántas facturas trae el documento.
//
// Si la IA falla --sin saldo, API caída, documento ilegible-- se queda lo del
// regex y la subida sigue como siempre. Nunca se pierde una factura por esto.
...
async function analizarFactura(buffer, esPdf, nombreOriginal) {
  const huella = crypto.createHash('sha256').update(buffer).digest('hex');
  const regex = await analizarBuffer(buffer, esPdf);
  // Lo que leyó el regex se guarda siempre, gane quien gane: donde no
  // coincida con la IA hay un fallo suyo que se puede arreglar mirando el
  // documento, en vez de suponiendo.
  const lecturaRegex = { importes: regex.importes, totales: regex.totales };

  const ia = await leerFacturaConIA(buffer, esPdf, nombreOriginal);
  if (!ia.ok) {
    return { ...regex, huella, lecturaRegex, leidoConIA: false, motivoIA: porQueNoLeyoLaIA(ia.error) };
  }

  const fechas = ia.facturas.map(f => f.fecha).filter(Boolean).map(f => new Date(f))
    .filter(d => !isNaN(d.getTime()));
  const importes = ia.facturas.map(f => f.importe);
  return {
    ...regex,
    importes,
    totales: importes,
    fechas: fechas.length ? fechas : regex.fechas,
    lecturaRegex,
    leidoConIA: true,
...
// ¿Leyeron lo mismo? Se compara el importe principal de cada uno, al céntimo.
// Si la IA no llegó a leer no hay nada que comparar.
function hayDiscrepancia(factura) {
  if (!factura.leido_con_ia || !factura.lectura_regex) return false;
  const deIA = montoCaracteristico(factura.totales, factura.importes);
  const deRegex = montoCaracteristico(factura.lectura_regex.totales, factura.lectura_regex.importes);
  if (deIA === null || deRegex === null) return deIA !== deRegex;
  return Math.abs(deIA - deRegex) > TOLERANCIA;
}
...
async function reprocesarFacturaConIA(facturaId) {
  ...
  const analisis = ia.ok
    ? {
        importes: ia.facturas.map(f => f.importe),
        totales: ia.facturas.map(f => f.importe),
        fechas: ia.facturas.map(f => f.fecha).filter(Boolean).map(f => new Date(f)).filter(d => !isNaN(d.getTime())),
        proveedor: ia.facturas.map(f => f.proveedor).filter(Boolean)[0] || null,
        textoMayus: `IA: leída${ia.facturas.length > 1 ? ` — ${ia.facturas.length} facturas dentro` : ''}${
          ia.facturas[0].proveedor ? ` — proveedor: ${ia.facturas[0].proveedor}` : ''}.`,
      }
    : { importes: [], totales: [], fechas: [], proveedor: null, textoMayus: `IA: no se ha podido leer (${ia.error})` };

  await asegurarColumnasMotivo();
  const { rows } = await query(
    `UPDATE facturas SET importes = $1, totales = $2, fechas = $3,
            proveedor = COALESCE($5, proveedor), huella = $6
     WHERE id = $4 RETURNING *`,
    [analisis.importes, analisis.totales, analisis.fechas.map(d => d.toISOString().slice(0, 10)), facturaId,
     analisis.proveedor, huella]
  );
  const factura = { ...rows[0], textoMayus: analisis.textoMayus };
  const resultado = await intentarMatch(factura);
  await aplicarEstado(factura.id, resultado);
...
module.exports = {
  ...
  siguienteNumero, asegurarColumnasMotivo, guardarTextosFaltantes, probarLecturaConIA, analizarFactura, hayDiscrepancia,
  facturaConMismaHuella, listarFacturasIncompletas, completarDatosFactura,
};
