...
// a la vez (no todo el trimestre de golpe) para que la pantalla pueda
// mostrar progreso real en vez de un único POST largo y ciego.
async function reprocesarFactura(facturaId) {
  const { rows: previas } = await query(`SELECT trimestre_id, ruta_blob, nombre_original FROM facturas WHERE id = $1`, [facturaId]);
  if (previas.length === 0) throw new Error('Factura no encontrada.');
  const { trimestre_id: trimestreId, ruta_blob: rutaBlob, nombre_original: nombreOriginal } = previas[0];
...
// Ids de las facturas de un trimestre a las que aplica reprocesarFactura --
// para que la pantalla sepa qué lista recorrer, una a una, con progreso.
async function listarFacturasSinResolver(trimestreId) {
  const { rows } = await query(
    `SELECT id FROM facturas WHERE trimestre_id = $1 AND estado IN ('sin_match', 'revisar') AND es_imagen = false`,
    [trimestreId]
  );
...
async function listarFacturasSinImporte(trimestreId) {
  const { rows } = await query(
    `SELECT id FROM facturas
     WHERE trimestre_id = $1 AND estado != 'matcheada'
...
async function confirmarDatosManual(facturaId, { importe, fecha }) {
  const { rows } = await query(
    `UPDATE facturas SET
     ...
     WHERE id = $1 RETURNING *`,
    [facturaId, importe ?? null, fecha ?? null]
  );
  if (rows.length === 0) throw new Error('Factura no encontrada.');
...
async function aplicarEstado(facturaId, resultado) {
  ...
  const estado = resultado.tipo === 'match_directo' ? 'matcheada'
    ...
    : 'revisar';
  await query(
    `UPDATE facturas SET estado = $2, motivo_tipo = $3, motivo_detalle = $4 WHERE id = $1 AND estado != 'matcheada'`,
    [facturaId, estado, resultado.tipo, resultado.detalle || null]
  );
}
