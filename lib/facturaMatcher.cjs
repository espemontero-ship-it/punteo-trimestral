...
async function facturasSinResolver(trimestreId, proveedorClave, excluirId) {
  const { rows } = await query(
    proveedorClave
      ...
      : `SELECT id, numero, totales, importes, fechas FROM facturas
         WHERE trimestre_id = $1 AND proveedor_clave IS NULL AND estado IN ('sin_match', 'revisar') AND id != $3`,
    proveedorClave ? [trimestreId, proveedorClave, excluirId] : [trimestreId, null, excluirId]
  );
  return rows;
}
...
async function intentarMatch(trimestreId, factura) {
  const { id: facturaId, numero, proveedor_clave: proveedorClave, totales, importes, fechas, es_imagen: esImagen, textoMayus } = factura;
...
module.exports = { procesarFacturaSubida, confirmarMatch, reintentarPendientes, siguienteNumero };
