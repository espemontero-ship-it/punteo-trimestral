...
// NO HAY MARGENES. Todo se compara en centimos enteros: o cuadra exacto, o no
// es un emparejamiento. Antes habia dos margenes -- un centimo en el
// emparejamiento simple y cincuenta en las combinaciones-- y por ahi entraban
// los emparejamientos que no cuadran: 124,74 EUR dados por buenos contra una
// linea de 125,00 EUR.
...
async function intentarMatch(factura) {
  ...
  // LAS DE UN COLABORADOR NO SE CRUZAN. Ese gasto lo pago el de su bolsillo:
  // de la cuenta no sale esa factura, sale el reembolso que se le hace, que
  // ademas suele juntar varias. Cruzarlas contra el banco solo puede encontrar
  // coincidencias falsas: una linea que por casualidad valga lo mismo.
  if (factura.subido_por) {
    return {
  ...
  const candidatos = pendientes
    .filter(m => centimosDeMovimiento(m) + centimos === 0)
  ...
  if (candidatos.length > 0) {
    return {
      ...
      detalle: candidatos.length === 1
        ? `Encaja con una línea de ${Math.abs(centimos / 100).toFixed(2)}€ — confírmalo para emparejarlas.`
        : `${candidatos.length} líneas pendientes encajan con ${Math.abs(centimos / 100).toFixed(2)}€ — elige a cuál corresponde.`,
    };
  }
  ...
  if (sumasInternas.length > 0) {
    ...
    for (const suma of sumasInternas) {
      for (const m of pendientes) {
        if (centimosDeMovimiento(m) + suma === 0 && !porSuma.has(m.id)) {
    ...
    if (porSumaLista.length > 0) {
      const sumaTexto = porSumaLista.length === 1 ? ` (${(porSumaLista[0].suma / 100).toFixed(2)}€)` : '';
      return {
        tipo: 'ambiguo', numero, facturaId, facturaConcepto: concepto,
  ...
  for (let tamano = 1; tamano <= MAX_FACTURAS_EN_COMBO - 1; tamano++) {
    for (const grupo of combinaciones(otras, tamano)) {
      ...
      for (const m of pendientes) {
        if (centimosDeMovimiento(m) + suma !== 0) continue;
        posibles.push({ grupo, suma, match: m, cuantas: tamano + 1 });
      }
    }
  }
  posibles.sort((a, b) => a.cuantas - b.cuantas);

  const mejor = posibles[0];
  if (mejor) {
    const { grupo, suma, match } = mejor;
    ...
    return {
      ...
      movimientoId: match.id, suma: suma / 100, exacto: true, diferencia: 0,
      otrasFacturas: grupo.map(o => ({ id: o.id, numero: o.numero, monto: o.monto })),
      detalle: `Esta factura (${monto.toFixed(2)}€) + ${otrasTexto} suman ${(suma / 100).toFixed(2)}€, que es justo la línea de ${Math.abs(match.importe).toFixed(2)}€.`,
    };
  }
...
async function procesarFacturaSubida({ hoja, clave, rutaBlob, nombreOriginal, concepto, analisis, subidoPor, proyectoId }) {
  ...
  const insert = await query(
    ...
    `INSERT INTO facturas (proveedor_clave, ruta_blob, nombre_original, numero,
                            importes, totales, fechas, concepto, estado, subido_por, proyecto_id,
                            proveedor, huella)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'sin_match',$9,$10,$11,$12)
     RETURNING *`,
    [
...
async function reintentarPendientes() {
  const { rows: pendientes } = await query(
    `SELECT * FROM facturas WHERE trimestre_id IS NULL AND estado IN ('sin_match', 'revisar')`
  );
  let resueltas = 0;
...
async function analizarFactura(buffer, esPdf, nombreOriginal, leer = leerFacturaConIA) {
  ...
  if (!ia.ok) {
    return { huella, importes: [], totales: [], fechas: [], leidoConIA: false, motivoIA: porQueNoLeyoLaIA(ia.error) };
  }
