...
async function reprocesarFactura(facturaId) {
  const { rows: previas } = await query(`SELECT trimestre_id, ruta_blob FROM facturas WHERE id = $1`, [facturaId]);
  if (previas.length === 0) throw new Error('Factura no encontrada.');
  const { trimestre_id: trimestreId, ruta_blob: rutaBlob } = previas[0];

  const buffer = await descargarBlob(rutaBlob);
  const analisis = await analizarBuffer(buffer, true);
  const { rows } = await query(
...
module.exports = {
  ...
  reprocesarFactura, listarFacturasSinResolver, siguienteNumero, asegurarColumnasMotivo,
};
