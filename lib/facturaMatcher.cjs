...
async function procesarFacturaSubida({ hoja, clave, rutaBlob, nombreOriginal, concepto, analisis, subidoPor, proyectoId }) {
  ...
  if (yaSubida) {
    ...
    return {
      tipo: 'duplicada',
      duplicada: { numero: yaSubida.numero, nombre: yaSubida.nombre_original },
      // Al colaborador no se le dice el número: esa numeración no es suya.
      detalle: subidoPor
        ? 'This file has already been uploaded. It has not been saved again.'
        : `Este archivo ya está subido como factura #${yaSubida.numero} (${yaSubida.nombre_original}). No se ha vuelto a guardar.`,
    };
  }
