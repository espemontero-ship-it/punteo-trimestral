  ...
  if (sumasInternas.length > 0) {
    ...
    if (porSumaLista.length === 1) {
      ...
      return {
        ...
        movimientoId: c.id, suma: c.suma / 100, exacto: true, diferencia: 0,
        otrasFacturas: [],
        detalle: `Este archivo trae varias facturas dentro y sus totales suman ${(c.suma / 100).toFixed(2)}€, que es justo la línea de ${Math.abs(Number(c.importe)).toFixed(2)}€. Confírmalo.`,
      };
    }
    if (porSumaLista.length > 1) {
      return {
