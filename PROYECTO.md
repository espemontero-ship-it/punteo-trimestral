...
- **Falta información en cada línea del checklist.** Ni el importe ni si ya tiene nota se ven sin expandir la tarjeta de `GrupoProveedor`. Con varias docenas de tarjetas, abrir una a una solo para ver el importe es la misma fricción que se quería quitar. Falta decidir qué mostrar en la fila colapsada (importe del grupo, si ya hay nota) antes de construirlo.
