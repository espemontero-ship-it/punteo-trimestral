...
- **Columnas base, en este orden**: Fecha, Concepto, Proveedor, Importe, Estado, Nota, Proyecto — **7 columnas, sin columna Factura separada** (decisión 2026-07-27: Factura pasa a ser parte de Estado, ver abajo).
...
- **Estado — desplegable siempre editable, con las 3 opciones (pendiente/pedida/resuelta)**, incluso cuando ya está resuelta: se puede revertir a mano una línea resuelta por error. Si la resolución tiene factura emparejada, debajo del desplegable aparece un enlace subrayado normal "ver factura" (sin flecha, sin color nuevo — el azul de acento que ya se usa en todos los enlaces). Si no tiene factura (resuelta con nota manual, ej. un ingreso), no hay enlace.
